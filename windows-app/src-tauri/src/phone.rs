// Ported from src-tauri/src/phone.rs. Pure serde + std; the only changes are
// the two places that start a process:
//
//   · curl goes through hidden_command. Every child-process spawn in this app
//     does, or a console window flashes — and this one runs on a timer while the
//     user is working, so it would flash repeatedly.
//   · the device name comes from COMPUTERNAME instead of the `hostname` binary:
//     same answer, no child process to hide.
...
fn device_name() -> String {
    // COMPUTERNAME rather than spawning `hostname`. Windows sets it for every
    // process, so this is the same answer without a child process at all — and
    // a spawn here would have to be hidden anyway or it flashes a console window
    // on a path that runs while the user is doing something else.
