...
impl AgentSessionData {
    ...
    fn get_summary(&self) -> serde_json::Value {
        ...
        serde_json::json!({
            ...
            // 粒子模式要往这个 agent 的窗口里发字,得先按 pid 把它激活。
            // 快照里原本只有拼进 id 字符串里的 pid,前端还得去切字符串 —— 那种
            // "把结构化数据编码进字符串再解出来"的地方,改一次 id 格式就会静悄悄地断。
            "pid": self.pid,
...
impl AgentMonitor {
    ...
    /// 粒子面板要的那份 transcript —— 和 `get_snapshot` 里的 `recentMessages` **不是**
    /// 同一份。
    ///
    /// 快照那份把 assistant 截到 120 字:它是喂仪表盘用的,一行摘要而已。可粒子面板是
    /// 拿来**读**的,120 字的断句读起来只有"这里本来有话"的感觉。所以这儿单开一个口子,
    /// 给到 1200 字、80 条 —— 也不去动快照,免得为了这一处把别处的载荷也撑大。
    pub fn transcript(&self, agent_type: &str, limit: usize) -> Vec<serde_json::Value> {
        ...
        s.messages.iter().rev().take(limit.clamp(1, 200)).rev().map(|m| {
            serde_json::json!({
                "role": m.role,
                "type": m.msg_type,
                "toolName": m.tool_name,
...
/// (db, db mtime, wal (mtime, len)) for every Cursor db — the whole input of
/// `read_cursor_conversations`. New messages land in `-wal` first and only
/// reach the main file at a checkpoint, so the main file's mtime alone would
/// hand back stale conversations.
type CursorFingerprint = Vec<(PathBuf, Option<SystemTime>, Option<(SystemTime, u64)>)>;

fn cursor_fingerprint(dbs: &[PathBuf]) -> CursorFingerprint {
    dbs.iter().map(|p| {
        let db = fs::metadata(p).ok().and_then(|m| m.modified().ok());
        let mut wal = p.clone().into_os_string();
        wal.push("-wal");
        let wal = fs::metadata(PathBuf::from(wal)).ok()
            .and_then(|m| Some((m.modified().ok()?, m.len())));
        (p.clone(), db, wal)
    }).collect()
}

/// Last result and the files it was read from. The scanner calls
/// `read_cursor_conversations` every 5s while Cursor is detected; nearly always
/// nothing has changed, and re-reading was the most expensive thing Terse did.
static CURSOR_CACHE: std::sync::LazyLock<std::sync::Mutex<Option<(CursorFingerprint, Vec<(String, String, String)>)>>> =
    std::sync::LazyLock::new(|| std::sync::Mutex::new(None));

/// The bubble JSON strings of one Cursor db, read in-process.
///
/// This used to be `sqlite3 -readonly -json …` in a child process. The CLI's
/// JSON writer is pathologically slow on large text values: measured on this
/// machine, 467 rows (3.2 MB) took 214 s and 146 s of CPU as JSON, against
/// 0.15 s as plain output — and the scanner started it again every 5 s, once
/// per workspace db, blocking the scan thread the whole time. The query and
/// the rows are unchanged; only the reader is.
fn cursor_bubble_values(db_path: &PathBuf) -> Result<Vec<String>, String> {
    use rusqlite::{types::ValueRef, OpenFlags};
    // Read-only, like the CLI's -readonly: Cursor holds this db open and writes to it.
    ...
    let mut stmt = conn.prepare(
        "SELECT value FROM cursorDiskKV WHERE key LIKE 'bubbleId%' AND length(value) > 200 ORDER BY rowid LIMIT 1000",
    ).map_err(|e| e.to_string())?;
...
fn read_cursor_conversations() -> Vec<(String, String, String)> {
    ...
    let fingerprint = cursor_fingerprint(&dbs_to_try);
    if let Some((fp, msgs)) = CURSOR_CACHE.lock().unwrap_or_else(|e| e.into_inner()).as_ref() {
        if *fp == fingerprint { return msgs.clone(); }
    }
    ...
    for db_path in &dbs_to_try {
        ...
        let values = match cursor_bubble_values(db_path) {
            Ok(v) => v,
            Err(e) => {
                eprintln!("[terse-agent] cursor db read failed on {:?}: {}", db_path.file_name().unwrap_or_default(), e);
                continue;
            }
        };
    ...
    *CURSOR_CACHE.lock().unwrap_or_else(|e| e.into_inner()) = Some((fingerprint, all_msgs.clone()));
    all_msgs
}
