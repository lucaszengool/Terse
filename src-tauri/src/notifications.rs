...
fn notify_desktop(app: &AppHandle, rec: &AlertRecord) {
    // When the dynamic island is on screen it IS the notification surface — the alert
    // morphs out of the pill instead of a second card appearing beside it. The toast
    // window is the fallback for when the island is hidden (no agents connected).
    let island_up = app
