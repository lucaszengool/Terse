...
fn navigate_to_social(app: AppHandle) {
    if let Some(win) = app.get_webview_window("main") {
        if let Ok(url) = "tauri://localhost/social.html".parse() {
            let _ = win.navigate(url);
        } else {
            let _ = win.eval("window.location.replace('/social.html');");
        }
    }
}
...
/// over MCP with `x-terse-identity`. If the app kept this value privately, the
/// card the agent drafted and the card this app shows would be two different
/// cards — and the bug would look like "my agent says it wrote my profile but
/// Terse says I have none". ~/.terse/social-identity is the handshake, and the
/// README prompt writes the very same file when the agent gets there first.
///
/// 0600, because it IS the credential: the server stores only its hash, so
/// whoever holds this string owns that card.
#[tauri::command]
