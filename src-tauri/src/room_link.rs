...
/// 小镇里把桌面上的文件丢到别人(或别人的小伙伴)身上:人亲手丢的,不用再问一遍"批不批准" ——
/// 但关卡一道不少:公开房间不传、符号链接不传、超 20 MB 不传、敏感路径不传、扫到密钥不传,
/// 加密房间照样先封再上传。和 agent 发文件走的是同一个 stage_share。
#[tauri::command(async)]
...
pub fn rl_fetch_file(room_id: String, key: String, room_name: String, file: Value, from: String,
    ...
    // 和浏览器下载的文件一样打上隔离标记:双击运行会被 Gatekeeper 拦下来问
    #[cfg(target_os = "macos")]
    {
