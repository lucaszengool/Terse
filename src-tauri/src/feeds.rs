...
fn notif_kind(bundle: &str) -> &'static str {
    if bundle.starts_with("com.apple.") && !crate::messages::is_chat_app(bundle) { "system" } else { "notif" }
}
...
mod mac {
    ...
    unsafe fn ns_str(s: id) -> Option<String> {
    ...
    /// Run `f` inside an autorelease pool. The scanner thread never returns to a
    /// run loop, so without this every autoreleased NSString it touches leaks.
    ...
    /// (bundle id, localized name) of a running process.
    pub fn app_of(pid: u32) -> Option<(String, String)> {
        unsafe {
            ...
            let b: id = msg_send![ra, bundleIdentifier];
            let n: id = msg_send![ra, localizedName];
            let bundle = ns_str(b)?;
            Some((bundle.clone(), ns_str(n).unwrap_or(bundle)))
        }
    }

    /// The installed app's display name for a bundle id ("Script Editor"),
    /// because the notification store only keeps a lower-cased bundle id.
    pub fn name_for_bundle(bundle: &str) -> Option<String> {
        unsafe {
            let ws: id = msg_send![class!(NSWorkspace), sharedWorkspace];
            let c = std::ffi::CString::new(bundle).ok()?;
            let s: id = msg_send![class!(NSString), stringWithUTF8String: c.as_ptr()];
            let url: id = msg_send![ws, URLForApplicationWithBundleIdentifier: s];
            if url == nil { return None; }
            let path: id = msg_send![url, path];
            let fm: id = msg_send![class!(NSFileManager), defaultManager];
            let name: id = msg_send![fm, displayNameAtPath: path];
            ns_str(name).map(|n| n.trim_end_matches(".app").to_string())
        }
    }

    /// Pids owning on-screen, normal-layer windows, front to back. Needs no
    /// permission: owner and bounds are public, only titles are gated.
    pub fn onscreen_pids() -> Vec<u32> {
        ...
        unsafe fn num(d: CFDictionaryRef, key: &str) -> Option<f64> {
            ...
            let mut out = 0f64;
            CFNumberGetValue(v as CFNumberRef, kCFNumberDoubleType, &mut out as *mut f64 as *mut c_void)
                .then_some(out)
        }
        ...
        unsafe {
            ...
            for i in 0..CFArrayGetCount(arr) {
                ...
                if d.is_null() || num(d, "kCGWindowLayer").unwrap_or(-1.0) != 0.0 { continue; }
                let Some(b) = ({
                    let k = CFString::new("kCGWindowBounds");
                    let v = CFDictionaryGetValue(d, k.as_concrete_TypeRef() as *const c_void);
                    (!v.is_null()).then_some(v as CFDictionaryRef)
                }) else { continue };
                // Toolbars, bubbles and input-method panels are not windows anyone reads.
                if num(b, "Width").unwrap_or(0.0) < 160.0 || num(b, "Height").unwrap_or(0.0) < 100.0 { continue; }
                let pid = num(d, "kCGWindowOwnerPID").unwrap_or(0.0) as u32;
                if pid == 0 || pid == me || out.contains(&pid) { continue; }
                out.push(pid);
                if out.len() >= 16 { break; }
            }
...
fn scan_windows(names: &mut HashMap<u32, (String, String)>) -> Vec<Seen> {
    ...
    if !crate::ax_read::is_trusted() { return seen; }
    let t = now();
    let pids = mac::pooled(mac::onscreen_pids);
    let mut current: HashMap<String, (String, String, Vec<String>)> = HashMap::new();
    for pid in pids {
        let info = match names.get(&pid) {
            Some(i) => i.clone(),
            None => match mac::pooled(|| mac::app_of(pid)) {
        ...
        let titles: Vec<String> = crate::ax_read::window_titles(pid, 6)
            ...
            .filter(|s| s.chars().count() >= 2 && *s != name)
            .collect();
...
fn pretty_name(bundle: &str) -> String {
    ...
    if known != bundle.rsplit('.').next().unwrap_or(bundle) { return known; }
    #[cfg(target_os = "macos")]
    if let Some(n) = mac::pooled(|| mac::name_for_bundle(bundle)) { return n; }
    known
}
        ...
        loop {
            ...
            if let Some(err) = child.stderr.take() {
                for line in BufReader::new(err).lines().map_while(Result::ok) {
                    let Ok(np) = serde_json::from_str::<NowPlaying>(line.trim()) else { continue };
                    if np.unsupported { unsupported = true; break; }
                    if np.title.is_empty() || np.bundle.is_empty() { continue; }
                    let lower = np.bundle.to_ascii_lowercase();
                    let name = if np.app.is_empty() { np.bundle.clone() } else { np.app.clone() };
                    MEDIA_SEEN.lock().unwrap_or_else(|e| e.into_inner()).push((lower.clone(), name.clone()));
                    push_item(FeedItem {
                        id: String::new(), source: format!("media:{lower}"), kind: "media".into(),
                        app_id: lower, app_name: name, sender: np.title, group: None,
                        body: np.artist, ts: now(),
                    });
                }
            }
            let _ = child.kill();
            let _ = child.wait();
            // This macOS has no MRNowPlayingRequest: nothing to retry.
            if unsupported { return; }
            // A reader that ran a while and then died is restarted promptly;
            // one that dies on launch backs off, then gives up.
            if now() - started > 60 { backoff = 5; failures = 0; } else { failures += 1; }
...
pub fn fix_permission(which: &str) {
    ...
    if which == "accessibility" {
        if crate::ax_read::prompt_for_trust() { return; }
    }
    let url = match which {
...
pub fn sources_json() -> serde_json::Value {
    ...
    serde_json::json!({
        ...
        "sources": list,
        "accessibility": crate::ax_is_trusted(),
    })
}
