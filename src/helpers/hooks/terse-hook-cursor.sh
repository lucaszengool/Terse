#!/bin/bash
# terse-hook-cursor.sh — Cursor beforeShellExecution hook adapter
# Cursor hooks receive JSON on stdin with { tool_name, tool_input } format
# and return { decision: "allow"|"deny", updatedInput? } on stdout.
#
# Install: Add to Cursor settings → Agent Hooks → beforeShellExecution
# Or configure in ~/.cursor/hooks.json
...
# Extract command — Cursor may use .command, .tool_input.command, or .input.command
...
# ── Shared skip/compress logic (same as Claude Code hook) ──
...
# Source shared compress patterns
...
# Wrap command to pipe through terse-compress
WRAPPED="($COMMAND) 2>&1 | node $TERSE_COMPRESS"

# Emit Cursor hook response format
jq -n \
  --arg cmd "$WRAPPED" \
  '{
    decision: "allow",
