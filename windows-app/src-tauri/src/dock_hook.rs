// Ported from src-tauri/src/dock_hook.rs, unchanged: it is std + serde + tauri
// throughout — a localhost hook endpoint and a per-session queue, with nothing
// platform-specific in it. It and room_link.rs call into each other, so they
// port as a pair.
