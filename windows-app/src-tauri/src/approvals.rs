//! Detect agent approval prompts (Windows: the same parser, reading window text
//! through UI Automation instead of macOS Accessibility — see ax_read::window_text).
//!
...
pub fn spawn_scanner(app: AppHandle) {
    std::thread::spawn(move || {
        ...
        // No permission gate here, unlike macOS: UI Automation reads another
        // app's window text without a grant, so there is nothing to ask for and
        // no state where sweeps have to pause and wait for the user.
        loop {
            scan_once(&app);
