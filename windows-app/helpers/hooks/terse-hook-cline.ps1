# terse-hook-cline.ps1 — Cline PreToolUse hook adapter (Windows)
...
if ($toolName -ne "execute_command" -and $toolName -ne "Bash") { exit 0 }

$command = if ($event.tool_input.command) { $event.tool_input.command } else { "" }
...
$output = @{ hookSpecificOutput = @{ hookEventName = "PreToolUse"; permissionDecision = "allow"; updatedInput = @{ command = $wrapped }; additionalContext = "" } } | ConvertTo-Json -Depth 4 -Compress
Write-Output $output
exit 0
